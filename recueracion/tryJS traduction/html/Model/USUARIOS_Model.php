...
function __construct($login,$password,$nombre,$apellidos,$email,$dni,$tlf,$bday,$alergias,$direccion,$cp,$sexo,$tipo_usuario){
	$this->login = $login;
...
//funcion que comprueba que el tipo_usuario sea admin o usuario
function comprobar_tipo_usuario()
{
	$array = array();
	$array[0] = 'tipo_usuario';

	$this->tipo_usuario = trim($this->tipo_usuario);

	if(empty($this->tipo_usuario)){//comprobamos si esta vacio
		$array[1] = "00001";
		$array[2] = "paramVacio";

		return $array;

	}else if($this->tipo_usuario != 'admin' && $this->tipo_usuario != 'usuario'){//comprobamos si coinicide con los campos
		$array[1] = "00100";
		$array[2] = "tipo_usuarioError";
...
function comprobar_codigoPostal()
{
	...
	}else if( !preg_match('/^\d{5}$/', $this->direccion) ){//comprobamos si coincide con la expresion esperada
		$array[1] = "00050";
		$array[2] = "errorCP";
...
function comprobar_atributos_ADD(){
	...
	if ($aux !== true) {// si el login no es correcto se almacena el error en la posicion 0
		$array[0] = $aux;
		$correcto = false;
	}

	$aux = $this->comprobar_password();
	if ($aux !== true) {// si la contraseña no es correcto se almacena el error en la posicion 1
		$array[1] = $aux;
		$correcto = false;
	}

	$aux = $this->comprobar_dni();// si el dni no es correcto se almacena el error en la posicion 2
	if ($aux !== true) {
		$array[2] = $aux;
		$correcto = false;
	}
	...
	if ($aux !== true) {// si el nombre no es correcto se almacena el error en la posicion 3
		$array[3] = $aux;
		$correcto = false;
	}
	$aux = $this->comprobar_apellidos();
	if ($aux !== true) {// si el apellido no es correcto se almacena el error en la posicion 4
		$array[4] = $aux;
		$correcto = false;
	}
	$aux = $this->comprobar_tlf();
	if ($aux !== true) {// si el tlf no es correcto se almacena el error en la posicion 5
		$array[5] = $aux;
		$correcto = false;
	} 
	$aux = $this->comprobar_email();
	if ($aux !== true) {// si el email no es correcto se almacena el error en la posicion 6
		$array[6] = $aux;
		$correcto = false;
	} 
	$aux = $this->comprobar_fecha();
	if ($aux !== true) {// si la fecha no es correcta se almacena el error en la posicion 7
		$array[7] = $aux;
		$correcto = false;
	}
	$aux = $this->comprobar_sexo();
	if ($aux !== true) {// si el sexo no es correcto se almacena el error en la posicion 8
		$array[8] = $aux;
		$correcto = false;
	}
	$aux = $this->comprobar_alergias();
	if ($aux !== true) {// si las alergias no son correctas se almacena el error en la posicion 9
		$array[9] = $aux;
		$correcto = false;
	}
	$aux = $this->comprobar_direccion();
	if ($aux !== true) {// si la direccion no es correcta se almacena el error en la posicion 10
		$array[10] = $aux;
		$correcto = false;
	}
	$aux = $this->comprobar_codigoPostal();
	if ($aux !== true) {// si el codigo postal n no es correcto se almacena el error en la posicion 11
		$array[11] = $aux;
		$correcto = false;
	}
...
function login(){
	...
	if ($resultado->num_rows == 0){
		...
		if ($tupla['PASSWORD'] == $this->password && $tupla['ACTIVADO'] == "activado"){
			return true;
		}
		else{
